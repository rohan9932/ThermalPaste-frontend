...
export function Sidebar() {
  const [activeId, setActiveId] = useState<string>("all-tech");

  return (
    <div className="w-64 h-[calc(100vh-57px)] bg-[#0F1117] border-r border-[#222834] flex flex-col py-10 px-4 select-none shrink-0 overflow-y-auto">
      {/* NAVIGATION SECTION */}
      <div className="space-y-2 mb-6">
        {/* need to work on active functionality later */}
        <div className="space-y-1">
          <button
            onClick={() => {}}
            className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all duration-150 cursor-pointer font-semibold text-sm ${"bg-[#222732] text-[#00D8F6]"}`}
          >
      ...
      <div>
        <div className="flex items-center justify-between px-3 mb-2">
          ...
          <button
            className="text-[#8F99A8] hover:text-white p-0.5 rounded transition cursor-pointer"
            title="Add Sub-group"
          >
